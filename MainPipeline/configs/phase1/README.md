...
Trong inference full pipeline, `generation.stage_a_batch_size` là số ảnh full-page gom trong mỗi lần Stage A generate, `generation.image_batch_size` là số ảnh gom trong lô bao ngoài, và `generation.stage_b_batch_size` là số crop trong mỗi lần Stage B generate. Để Stage A batch thật, đặt `image_batch_size` >= `stage_a_batch_size`. Với config YOLO layout, `stage_a_batch_size` không có tác dụng vì Stage A bị bỏ qua.
...
Nếu predictions CSV có row lỗi, chạy `python -m MainPipeline.src.phase1.clean_prediction_errors --config <inference-config>`, sau đó chạy lại `infer_phase1 --resume` để chỉ xử lý lại các ảnh lỗi đã bị xóa khỏi CSV.
