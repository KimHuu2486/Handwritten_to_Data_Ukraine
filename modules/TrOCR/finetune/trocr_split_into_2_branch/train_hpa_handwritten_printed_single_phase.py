...
# -*- coding: utf-8 -*-
"""
Single-phase fine-tuning for the RUKOPYS HPA handwritten+printed branch.

Branch:
  - labels: handwritten, printed
  - base model: Kansallisarkisto/cyrillic-htr-model
...
# Branch constants
# -----------------------------------------------------------------------------
BRANCH_NAME = "handwritten_printed"
TARGET_LABELS = ("handwritten", "printed")
MODEL_NAME = "Kansallisarkisto/cyrillic-htr-model"
BASE_PROCESSOR_MODEL = "microsoft/trocr-base-handwritten"
RUN_NAME = "hpa_handwritten_printed_single_phase"

# Ukrainian is not Russian; these are added explicitly for this Cyrillic branch.
UKRAINIAN_EXTRA_TOKENS = ["і", "ї", "є", "ґ", "І", "Ї", "Є", "Ґ"]

# Official/special markup tokens used in RUKOPYS text labels.
...
OUTPUT_DIR = Path("/home/ubuntu/outputs_hpa_handwritten_printed_single")
...
def finalize_config(args: SimpleNamespace) -> None:
    ...
    args.final_dir = env_path(("HPA_FINAL_DIR",), args.final_dir) or args.output_dir / "final_cyrillic_handwritten_printed_model"
...
def load_processor(args: SimpleNamespace, hf_token: Optional[str], cache_kwargs: Dict[str, Any]) -> TrOCRProcessor:
    if args.resume_model_dir:
        try:
            processor = TrOCRProcessor.from_pretrained(str(args.resume_model_dir))
            print(f"[INFO] Loaded processor from resume model dir: {args.resume_model_dir}")
            return processor
        except Exception as exc:
            print(f"[WARN] Could not load processor from resume model dir: {exc}")
    ...
    for kwargs in ({}, {"subfolder": "processor"}):
        try:
            ...
            print(f"[INFO] Loaded TrOCRProcessor from {args.model_name} with kwargs={kwargs}")
            return processor
        except Exception as exc:
            processor_errors.append(f"{kwargs}: {exc}")

    print("[WARN] Could not load full TrOCRProcessor from model repo:")
    for error in processor_errors:
        print(f"       {error}")
    print(f"[INFO] Falling back to image processor from: {args.base_processor_model}")
    image_processor = from_pretrained_hf(ViTImageProcessor, args.base_processor_model, hf_token, use_fast=False, **cache_kwargs)
    ...
    for kwargs in ({}, {"subfolder": "processor"}):
        try:
            ...
            print(f"[INFO] Loaded tokenizer from {args.model_name} with kwargs={kwargs}")
    ...
    for error in tokenizer_errors:
        print(f"       {error}")
    if args.model_name != args.base_processor_model:
        raise RuntimeError(
            f"Could not load tokenizer from {args.model_name}. Refusing to fall back to "
            f"{args.base_processor_model}, because that tokenizer does not match the decoder."
        )
    print(f"[INFO] Falling back to tokenizer from: {args.base_processor_model}")
    tokenizer = from_pretrained_hf(AutoTokenizer, args.base_processor_model, hf_token, use_fast=True, **cache_kwargs)
    return TrOCRProcessor(image_processor=image_processor, tokenizer=tokenizer)
...
def strip_text_markers(text: str) -> str:
    ...
    # [illegible] -> empty token placeholder removed
...
def build_training_args(args: SimpleNamespace) -> Seq2SeqTrainingArguments:
    ...
    kwargs: Dict[str, Any] = {
        ...
        "generation_max_length": min(args.max_target_length, 128),
...
def main() -> None:
    ...
    processor = load_processor(args, hf_token, cache_kwargs)
