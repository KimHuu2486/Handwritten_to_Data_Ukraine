...
- "bbox": [x1, y1, x2, y2] pixel coordinates of the bounding box for that specific line
...
def run_inference_single_image(model, processor, img_path, device):
    ...
    # Few-shot example image and ground truth
    few_shot_img = "/kaggle/input/datasets/quii29/rukopys-dataset/train/images/09ba34df-2665-452c-9ef4-6998a5e7944c.jpg"
    
    few_shot_output = """[
  {"bbox": [1213, 350, 2052, 530], "type": "handwritten", "text": "Магія голосу."},
  {"bbox": [395, 466, 2987, 866], "type": "handwritten", "text": "Спочатку був \\"брехунець\\". З нього почи-"},
  {"bbox": [151, 664, 2850, 1046], "type": "handwritten", "text": "нався день; клац - пронизливе рокотання"}
]"""

    messages = []
    
    # Add few-shot example if it exists
    if os.path.exists(few_shot_img):
        messages.append({
            "role": "user",
            "content": [
                {
        ...
        messages.append({
            "role": "assistant",
            "content": [
                {"type": "text", "text": few_shot_output}
            ]
        })
    else:
        print(f"⚠️ Không tìm thấy ảnh Few-shot {few_shot_img}, sẽ chạy Zero-shot.")
